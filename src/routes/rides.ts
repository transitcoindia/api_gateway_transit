...
ridesRouter.post('/request', async (req, res) => {
  try {
    ...
    const backendUrl = RIDER_BACKEND_URL.replace(/\/$/, '');
    const response = await axios.post(`${backendUrl}/api/rider/request`, req.body, {
      headers: {
    ...
    try {
      ...
      if (rideId) {
        ...
        const riderId = data.riderId ?? (req as any)?.user?.id ?? req.body?.riderId;
        if (!riderId) {
